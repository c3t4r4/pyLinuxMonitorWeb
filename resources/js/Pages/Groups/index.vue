    ...
    <DefaultPanel title="Grupos">
        <TableContent>
            <template #headerPage>
                <div class="flex-1 mt-3 px-2 flex justify-center lg:ml-6 lg:justify-end">
                    ...
                    <div class="ml-6 max-w-lg  lg:max-w-xs">
                        <Link as="button" :href="route(`groups.create`)" class="inline-flex items-center justify-center rounded-md border border-transparent bg-green-700 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-green-800 focus:outline-none focus:ring-2 focus:ring-green-800 focus:ring-offset-2 sm:w-auto hover:-translate-y-1 hover:scale-110 duration-300">Novo</Link>
                    </div>
                </div>
            </template>
            ...
            <TableModel>
                ...
                <tr v-for="(item, itemIndex) in groups.data" :key="itemIndex" class="divide-x divide-gray-300" :class="[itemIndex % 2 !== 0 ? 'bg-gray-50 hover:bg-gray-200' : 'bg-white hover:bg-gray-200']">
                    ...
                    <td class="whitespace-nowrap text-center py-4 pl-4 pr-4 text-sm sm:pr-6">
                        <div class="flex justify-center items-center space-x-2">
                            <TableButton type="show" :routeitem="route(`groups.show`, item.id)" method="get" />
                            <TableButton type="edit" :routeitem="route(`groups.edit`, item.id)" method="get" />
                            <TableButton type="delete" :routeitem="route(`groups.destroy`, item.id)" method="delete" />
                        </div>
                    </td>
                </tr>
            ...
            <template #pagination>
                <TablePagination :objectData="groups"/>
            </template>
        </TableContent>
    </DefaultPanel>
</template>
